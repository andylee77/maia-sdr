...
The build script (`build_fpga.bat`) runs on Windows and calls Vivado directly,
bypassing the ADI HDL Linux Makefile system (which requires `flock`).
...
This pulls `ext/maia-sdr/` (provides maia_hdl package) and the ADI HDL library.

## Build Pipeline

```
Amaranth HDL (p25_hdl/) ──→ Verilog (p25_core.v)
                               ↓
...
Checks that `ext/maia-sdr/maia-hdl/adi-hdl/` is populated. If not, runs
`git submodule update --init --recursive`.

### Step 2: Generate Verilog

- **P25 core**: `python3 -m p25_hdl.p25_top --config default` → `ip/p25-core/default/p25_core.v`
...
Copies project files into the maia-sdr tree at
`ext/maia-sdr/maia-hdl/projects/fishball7020_p25/` so that relative paths
in the pluto base design (`../pluto/system_bd.tcl`) resolve correctly.

Key files staged:

- `system_project.tcl` — generated, targets xc7z020clg400-1
- `system_bd.tcl` — copied from `projects/fishball7020_p25/system_bd_intree.tcl`
...
```
vivado ext\maia-sdr\maia-hdl\projects\fishball7020_p25\fishball_p25.xpr
```
...
```
del /s /q ext\maia-sdr\maia-hdl\adi-hdl\library\*\component.xml
build_fpga.bat
```
...
| `system_top.bit` | `ext/.../fishball_p25.runs/impl_1/` | Raw bitstream |
| `p25_core.v` | `ip/p25-core/default/` | Generated Verilog (19k lines) |
...
- **p25-httpd source**: Mounted from the local fishball-p25 repo at
  `/mnt/fishball-p25` (read-only). The Buildroot package uses `SITE_METHOD = local`.
...
- **Private repo**: fishball-p25 is private on GitHub. The build uses a local
  mount instead of git clone to avoid authentication issues in Docker.
