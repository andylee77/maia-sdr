...
Build a standalone P25 Phase 1 trunking radio on the Fishball Z7020 (Zynq-7020 + AD9361). FPGA handles DSP (channelization, C4FM demod, symbol timing). ARM PS handles control channel protocol (TSBK parsing, talkgroup tracking, voice grant management). Uses SDRTrunk as the reference architecture. Reuses Maia SDR's AD9361 core, DDC, DMA, and register infrastructure via submodule.
...
```
fishball-p25/
  ext/maia-sdr/                     # git submodule (provides maia_hdl Python package)
  p25_hdl/                          # Amaranth HDL source
...
- `ext/maia-sdr/` submodule provides `maia_hdl` Python package (DDC, DMA, registers, spectrometer, recorder, CDC)
...
2. Vivado IP packaging: `cd ip/p25-core && make`
3. Bitstream: `cd projects/fishball7020_p25 && make`
...
**C4FM Demodulator** (`c4fm_demod.py`):
...
### Top Level (`p25_top.py`)

Data flow:
```
AD9361 IQ (12-bit) -> RxIQCDC -> DDC (tune + decimate to 48 kSPS)
  -> C4FMDemod (discriminator) -> SymbolTimingRecovery (4800 sym/sec dibits)
  -> DibitPacker -> DmaStreamWrite -> DDR3 -> PS reads via UIO
```

Keep Spectrometer + Recorder from Maia as debug tools.
...
| Spectrometer (debug) | 16 | ~5000 | 12 |
| AXI + misc | 0 | ~3000 | 0 |
| **Total** | **~34** | **~15300** | **~21** |
| **Z7020 capacity** | **220** | **53200** | **140** |
| **Utilization** | **15%** | **29%** | **15%** |

Plenty of headroom for Phase 3 second channel (+17 DSP).

### Vivado Project

Based on `fishball7020_iio/system_bd.tcl` — same PS7, AD9361 LVDS, clocking wizard (62.5/125/187.5 MHz). Replace `maia_sdr` IP with `p25_core` IP. Same AXI HP port connections for DMA.
...
- RX LO: center of P25 system band (e.g., 852.5 MHz for 851-854 MHz system)
- Sample rate: 6.144 MSPS, BW: ~5 MHz
- Gain: slow AGC
...
- Add `DDC('clk3x')` + `C4FMDemod` + `SymbolTimingRecovery` + `DibitPacker` for traffic channel
- Independent register bank for PS to retune on grant detection
- Separate DMA channel to PS
- Additional cost: ~17 DSP48E1, stays well under 30% utilization

### Grant Following Latency

1. TSBK received: ~20ms
2. PS processes grant: ~1ms
3. DDC retune (register write): ~1µs
...
| Risk | Mitigation |
|------|-----------|
| C4FM demod AGC (disc output scales with |x|²) | Digital AGC before demod, or adaptive thresholds in PS |
...
1. **Phase 0**: Repo, submodules, build scripts. Skeleton `p25_top.py` wrapping Maia DDC + spectrometer. Build bitstream, boot on Fishball. ✅
2. **Phase 1A**: `C4FMDemod` + `SymbolTimingRecovery` in Amaranth. Simulation tests with synthetic C4FM. ✅
3. **Phase 1B**: Integrate into `p25_top.py`, add dibit packer + DMA. Test on hardware with recorded P25 IQ. ✅
4. **Phase 2A**: Rust TSBK parser + control channel state machine. Test with captured dibit streams. ✅
5. **Phase 2B**: Web UI, integration test with live P25 system. ✅
6. **Phase 3**: Second DDC/demod chain, traffic manager, voice extraction, audio. ✅
7. **Phase 4**: Bitstream build with Vivado. Build script (`build_fpga.bat`), XSA export. ✅
8. **Phase 5**: Tezuka firmware integration. P25 board config, SD card boot, hardware test. ← NEXT

### Critical Maia Files to Reference

- `maia-hdl/maia_hdl/maia_sdr.py` — top-level pattern to follow
- `maia-hdl/maia_hdl/ddc.py` — DDC module to reuse
- `maia-hdl/maia_hdl/dma.py` — DMA infrastructure to reuse
- `maia-hdl/maia_hdl/register.py` — register framework to reuse
- `maia-hdl/projects/fishball7020_iio/system_bd.tcl` — Vivado project to adapt
